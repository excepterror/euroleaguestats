...
class LoadingMessage(BoxLayout):
    ...
    def repeat_animations(label, delay):
        ...
        def start_animation(*args):
            anim = Animation(opacity=1, font_size=App.get_running_app().font_scale * sp(112), duration=.5) + Animation(font_size=App.get_running_app().font_scale * sp(70), duration=.25)
            anim.start(label)
